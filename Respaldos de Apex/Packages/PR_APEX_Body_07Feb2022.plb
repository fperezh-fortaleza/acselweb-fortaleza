...
begin 
  ...
  IF cTipoPdcion != 'A' THEN 
    ...
    IF cTipoRep = 'CLAU' THEN 
       return PR_APEX.IMPRIMIR_CLAUSULAS_TEXT (nIdePol, nIdeOp, cIdeRep, nCantidadCopia);
    END IF; 
    ...
    IF cTipoRep = 'MARF' THEN  
       RETURN PR_APEX.IMPRIMIR_MARBETE_FLOTILLA (nIdepol, nIdeop , NULL, cIdeRep , NULL, nCantidadCopia );
    END IF; 
    IF cTipoRep = 'CANC' THEN 
       return PR_APEX.IMPRIMIR_CANCELACION( nIdePol, nIdeop,NULL,cIdeRep,NULL,nCantidadCopia,cCodProd); 
    END IF;  
    ...
    IF cTipoRep = 'PAGO' THEN 
       return PR_APEX.IMPRIMIR_CONDICION_PAGO(nIdePol, NULL, cIderep, NULL, nCantidadCopia, nIdeOp); 
    END IF; 
  ...
  IF cTipoRep = 'NOCR' THEN  -- Revisar con Efrain. Varias Ocurrencias. 
    return PR_APEX.IMPRIMIR_NOTA_CREDITO (nIdePol, nIdeop, NULL, cIdeRep, NULL, nCantidadCopia);
  END IF; 
...
function imprimir_recibo (nIdePol NUMBER, nIdeop NUMBER, cTipo VARCHAR2, cReporte VARCHAR2, cNombreImpresora VARCHAR2,  
 ...
 nNumCert   NUMBER(10):=1;   --Asi viene del Acsel.
 cUrl       VARCHAR2(2000);                         
begin 
  ...
  ELSE 
	IF PR_POLIZA.NROCOPIAPOL(nIdePol,cReporte,null,null) = 1 THEN -- primera vez que se imprime la poliza 
	   cSelloAgua := 'ORIGINAL'; 
	ELSE 
	   cSelloAgua := 'COPIA'; --||to_char(v_active_copy-1); 
	END IF; 
  END IF; 
	  
  --Regenerar Condiciones 'RECI'lares antes de imprimir. 
  ...
  IF cReporte IS NOT NULL  THEN 
     cUrl := '&report='     ||cReporte
          || '&P_POLIZA='   ||to_char(nIdePol)
          || '&P_NUMCERT='  ||to_char(nNumCert)
          || '&COPIES='     ||to_char(nCantidadCopia)
          || '&P_IDEOP='    ||to_char(nIdeOp)
          || '&P_SELLOAGUA='||cSelloAgua
          || '&P_LOGO='     ||'S';    ---Hay que colocar un campo lista para seleccionar con logo o sin logo.
  END IF; 
  return cUrl;
end imprimir_recibo;  
...
function imprimir_coaseguro (nIdePol NUMBER, nIdeOp NUMBER, cTipo VARCHAR2, cReporte VARCHAR2,  
 ...
 nIdeRec  NUMBER(14);           -- Pedro R. Piña² 28/08/2000  ojo...Nunca Recibe Valor...
 cUrl     VARCHAR2(2000);
BEGIN 
   BEGIN 
     SELECT DISTINCT 'S' 
   ...
   EXCEPTION 
      WHEN NO_DATA_FOUND THEN 
           cExiste := 'N'; 
   END; 
   IF cExiste = 'S' AND cReporte IS NOT NULL THEN 
      cUrl := '&report='     ||cReporte
           || '&P_IDEPOL='   ||to_char(nIdePol)
           || '&COPIES='     ||to_char(nCantidadCopia)
           || '&P_IDEREC='   ||to_char(nIdeRec);
   END IF; 
   RETURN cUrl;
...
BEGIN 
 ...
 IF cReporte IS NOT NULL  THEN
    cUrl := '&report='||cReporte
         ...
         || '&P_CODPROD='||cCodProd;
 END IF; 
 --raise_application_error(-20100,'Valor: '||cReporte||' - '||cUrl );
 return cUrl; 
...
BEGIN 
  ...
  IF cReporte IS NOT NULL THEN 
     cUrl := '&report='         ||cReporte
          ...
          || '&P_FECVIG='       ||TO_CHAR(dFecVig,'DD/MM/YYYY')
...
function imprimir_condicion_pago (nIdePol NUMBER, cTipo VARCHAR2, cReporte VARCHAR2,  
                                  cNombreImpresora VARCHAR2, nCantidadCopia NUMBER,
                                  nIdeOp NUMBER) return varchar2 is 
   cParam1      VARCHAR2(1); 
   cFacturacion varchar2(1); 
   cParam       long; 
   cSelloAgua   varchar2(20); 
   cCodProd     POLIZA.CodProd%TYPE;
   cReporte1    VARCHAR2(20);
   cUrl         VARCHAR2(2000);
BEGIN 
    ...
    IF PR_ACCESO_USUARIO.AUTORIZA_OPERACION(SUBSTR(CODUSER,1,8),920) =  'S'  THEN  
       cSelloAgua := 'ORIGINAL';
    ELSE
       IF cFacturacion = 'P' THEN
          cReporte1 :='mxpoliz2'; 
       ELSE
          cReporte1 :='mxcertif'; 
       END IF;
       IF PR_POLIZA.NroCopiaPol(nIdePol,cReporte1,null,null) = 1 THEN 
            -- primera vez que se imprime la poliza
               cSelloAgua := 'ORIGINAL';
       ELSE
               cSelloAgua := 'COPIA'; 
       END IF;
     ...
     IF cFacturacion = 'P' THEN 
        cUrl     := '&report=mxpoliz2'
                 || '&PNUMOPER='||to_char(nIdeOp)
                 || '&P_SELLOAGUA='||cSelloAgua;
     ELSIF cFacturacion = 'C' THEN  
           IF cCodProd in ('TRA','TCR','TOA','RMQC','EMAC','AUC','INA') THEN
              cUrl := '&report=mxpoliz3'
                   || '&PNUMOPER='||to_char(nIdeOp)
                   || '&P_NUMCERT='||'1'
                   || '&P_SELLOAGUA='||cSelloAgua
                   || '&P_FDIGITAL=CONFIRMADIGITAL';
           END IF;
     END IF;
     return cUrl;
...
BEGIN 
  IF cReporte IS NOT NULL THEN 
     cUrl := '&report='       ||cReporte
          || '&P_NUMACUERDO=' ||to_char(nNumAcuerdo)
          || '&COPIES='       ||TO_CHAR(nCantidadCopia);
  END IF;
  RETURN cUrl; 
...
BEGIN 
  IF cReporte IS NOT NULL  THEN 
     cUrl := '&report='        ||cReporte
          || '&p_Fecha_desde=' ||TO_CHAR(dFecha_Desde,'DD/MM/YYYY')
          || '&p_Fecha_hasta=' ||TO_CHAR(dFecha_Hasta,'DD/MM/YYYY')
          || '&p_producto='    ||cCodProd
          || '&p_numpol='      ||TO_CHAR(nNumPol);
  END IF;  
  RETURN cUrl;
...
function imprimir_clausulas_text ( nIdePol NUMBER, nIdeOp NUMBER, cReporte VARCHAR2, 
                                   nCantidadCopia NUMBER ) return varchar2 is
cParam1   VARCHAR2(1);
cExiste   VARCHAR2(1) := 'N';   
cParam    VARCHAR2(2000);  
cUrl      VARCHAR2(2000);
BEGIN
   BEGIN
      SELECT DISTINCT 'S'
        INTO cExiste
        FROM CLAU_CERT cc, certificado c, poliza p
       WHERE cc.Idepol = nIdePol
         and cc.numoper = nIdeop
         and p.IdePol = cc.IdePol
         and p.stspol = 'ACT'
         and c.idepol = cc.idepol
         and c.numcert = cc.numcert
         and c.stscert = 'ACT';
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         cExiste := 'N';
   ...
   END IF; 
   return cUrl;
end imprimir_clausulas_text;

function imprimir_marbete_flotilla (nIdepol NUMBER, nIdeop NUMBER, cTipo VARCHAR2, cReporte VARCHAR2, 
                                    cNombreImpresora VARCHAR2, nCantidadCopia NUMBER) return varchar2 is
dummy          Number;
cParam1        VARCHAR2(1);
nCertInicial   NUMBER(5);
nContadorCert  NUMBER(5);
nContadorTotal NUMBER(5);
nCantCert      NUMBER(5);
cParam         VARCHAR2(2000);  -- Variable agregada para pase de Parámetro --
CURSOR c_ImpFlot IS
   SELECT UNIQUE b.NumCert
   FROM Recibo b, Certificado a, Cert_Veh c
   WHERE b.IdePol  = a.IdePol
   AND b.NumCert   = a.NumCert
   AND b.IdePol    = c.IdePol
...
begin
   ...
   nCantCert := FR_TOTAL_CERT(nIdePol,nIdeop);
   IF nCantCert >= 5 THEN   
      FOR ImpFlot IN c_ImpFlot LOOP
         nContadorCert  := nContadorCert  + 1;
         nContadorTotal := nContadorTotal + 1;
         IF nCertInicial IS NULL THEN 
            nCertInicial  := ImpFlot.NumCert;
         END IF;
         IF nContadorCert = 5 THEN
            IF cReporte IS NOT NULL  THEN           
               IF Reports_Is_Installed THEN
                  pl_id := Create_Parameter_List('tmpdata'); 
                  Add_Parameter(pl_id,'DESTYPE',TEXT_PARAMETER,cTipo); 
                  IF cNombreImpresora IS NOT NULL THEN
                     Add_Parameter(pl_id,'DESNAME',TEXT_PARAMETER,cNombreImpresora); 
                  ...
                  Add_Parameter(pl_id,'COPIES',TEXT_PARAMETER,nCantidadCopia); 
                  Add_Parameter(pl_id,'P_POLIZA',TEXT_PARAMETER,TO_CHAR(nIdepol)); 
                  Add_Parameter(pl_id,'P_NUMCERTINI',TEXT_PARAMETER,TO_CHAR(nCertInicial));
                  Add_Parameter(pl_id,'P_NUMCERTFIN',TEXT_PARAMETER,TO_CHAR(ImpFlot.NumCert));
                  Add_Parameter(pl_id,'P_IDEOP',TEXT_PARAMETER,TO_CHAR(nIdeOp));
                  Add_Parameter(pl_id,'PARAMFORM',TEXT_PARAMETER,'NO'); 
                  
                  -- Linea de Código desactivada para Prueba de nueva metodología de pase de parámetros **/                  
                  --> rp2rro.rp2rro_run_product(reports,cReporte,ASYNCHRONOUS,RUNTIME,filesystem,pl_id,cParam1);

                  -- Concatena Agrega los parámetros correspondientes a la variable de parámetros --
                  --cParam:= '&COPIES='||TO_CHAR(nCantidadCopia)||'&P_POLIZA='||TO_CHAR(nIdepol)||'&P_NUMCERTINI='||TO_CHAR(nCertInicial)
                  --       ||'&P_NUMCERTFIN='||TO_CHAR(ImpFlot.NumCert)||'&P_IDEOP='||TO_CHAR(nIdeOp);
              ...
              /*    
                  Destroy_Parameter_List( pl_id ); 
               ELSE
                  dummy:=ALERTA('Oracle Reports NO ESTA INSTALADO...');
               END IF;
            END IF; 
            nCertInicial    := NULL;
            nContadorCert   := 0;
         END IF;
         IF nContadorTotal = nCantCert AND nContadorCert > 0 THEN 
            IF cReporte IS NOT NULL  THEN
               IF Reports_Is_Installed THEN
                  pl_id := Create_Parameter_List('tmpdata'); 
                  Add_Parameter(pl_id,'DESTYPE',TEXT_PARAMETER,cTipo); 
  ...
  END IF;
  */
end imprimir_marbete_flotilla;

function imprimir_nota_credito (nIdePol NUMBER, nIdeop NUMBER, cTipo VARCHAR2, cReporte VARCHAR2,
                                cNombreImpresora VARCHAR2, nCantidadCopia NUMBER) return varchar2 is
dummy           Number;
cParam1         VARCHAR2(1);
nIdeNotaCredito NUMBER(14);
cParam          VARCHAR2(2000);  -- Variable agregada para pase de Parámetro --
CURSOR Credito IS
  SELECT DISTINCT NC.IDENOTACRED
    FROM RECIBO R, NOTA_CREDITO NC
   WHERE R.IDEPOL = nIdepol
    AND R.IDEOP    = nIdeop
    AND R.IDEOP    = NC.IDEOP
    AND NUMOBLIG IS NOT NULL;                                
begin
   IF cReporte IS NOT NULL  THEN
      RETURN NULL;
      /*
      IF Reports_Is_Installed THEN
      	 FOR c IN Credito LOOP
            BEGIN
--               IF cTipo='PRINTER' THEN
                   pl_id := Create_Parameter_List('tmpdata'); 
                   Add_Parameter(pl_id,'DESTYPE',TEXT_PARAMETER,cTipo); 
                   IF cNombreImpresora IS NOT NULL THEN
                      Add_Parameter(pl_id,'DESNAME',TEXT_PARAMETER,cNombreImpresora); 
                   END IF;
                   Add_Parameter(pl_id,'COPIES',TEXT_PARAMETER,nCantidadCopia); 
                   Add_Parameter(pl_id,'P_IDENOTACRED',TEXT_PARAMETER,TO_CHAR(C.IdeNotaCred)); 
                   Add_Parameter(pl_id,'P_TIPO',TEXT_PARAMETER,'PRINTER'); 
                   Add_Parameter(pl_id,'PARAMFORM',TEXT_PARAMETER,'NO'); 
                   rp2rro.rp2rro_run_product(reports,cReporte,ASYNCHRONOUS,RUNTIME,filesystem,pl_id,cParam1);
                   Destroy_Parameter_List( pl_id );
               ELSE    
                   -- Concatena Agrega los parámetros correspondientes a la variable de parámetros --
                   cParam:= '&COPIES='||TO_CHAR(nCantidadCopia)||'&P_IDENOTACRED='||TO_CHAR(C.IdeNotaCred)||'&P_TIPO=CACHE';
                   CALL_IMPRIMIR(cReporte,cParam); -- Invoca el procedimiento que llama el reporte y pasa la variable que contiene los parámetros --               
               END IF;    
            END;
         END LOOP;   
      ELSE
         dummy:=ALERTA('Oracle Reports NO ESTA INSTALADO, No se Imprimira la Nota de Crédito...');
      END IF;  
      */          
   END IF; 
end imprimir_nota_credito;
