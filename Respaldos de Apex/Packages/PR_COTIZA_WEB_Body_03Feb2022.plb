...
procedure plantilla_cotizacion ( nIdeCotizaPlanilla number, nIdeCotiza number) is 
 cursor cur_coberturas is  
    select codramoplan, codcobert, indobliga, prima, tasa, suma_asegurada, porcded, mtoded 
...
SELECT IdCotiza, TipoId, SerieId,NumId, DvId, Nombre, Email, Telefono, NroMovil, 
           ...
           Apellido, IdCotizaPlantilla,CodOfi,CodUsr,PlanAcuerdo
FROM COTIZACION  
...
BEGIN 
  ...
  FOR P IN COTI LOOP 
    ...
    dFecIniVig      :=PR.FECHA_EMISION(SYSDATE);--P.FecDesde; 
    dFecFinVig      :=ADD_MONTHS(PR.FECHA_EMISION(SYSDATE),12);--P.FecHasta; 
    ...
    cPlanFraccion   :=P.PlanAcuerdo;
    dVcto1raCuota   :=ADD_MONTHS(PR.FECHA_EMISION(SYSDATE),1);
   ...
   -- raise_application_error(-20100,'No se ha Definido el Corr'||cCodInter);
    ...
    INSERT INTO POLIZA(tipocotpol,codpol,numpol,numren,idepol,indnumpol,codprod,stspol, 
                ...
                VALUES('P',cCodProd,nNumPolDef,0,nIdePol,'N',cCodProd,'VAL', 
                      cCodCli,dFecFinVig,dFecIniVig,dFecFinVig,'A','P','M','N','P', 
                      cCodOfiEmi,cCodOfiSusc,cCodMoneda,'N','N','S','I', 
                      'A',dFecIniVig,'000024',cRespCob,cCodGrp,NULL,Trunc(sysdate),Trunc(sysdate), 
                      'N','N','N','N','N',1,'N', 
    ...
    BEGIN 
    INSERT INTO Plan_Fracc_Pol(ideplanfracc,idepol,numcert,numoper,fecvenpricuo,fecactualizacion) 
                        VALUES(cPlanFraccion,nIdePol,nNumCert,NULL,ADD_MONTHS(PR.FECHA_EMISION(SYSDATE),1),SYSDATE); 
    EXCEPTION 
      WHEN OTHERS THEN 
       --raise_application_error(-20100,'Error Insertando el Certificado ' ||nNumCert||sqlerrm); 
        NULL; 
    ...
    BEGIN 
     INSERT INTO CERTIFICADO(IdePol, NumCert, StsCert, DescCert, FecIng, CodCli, 
                      ...
                      VALUES(nIdePol,nNumCert, 'VAL', 'COTIZACION',dFecIniVig, cCodCli, 
      ...
      BEGIN 
          ...
          VALUES 
               ( nIdePol, nNumCert, cCodModelo, cCodMarca, NVL(cCodVersion,'01'),cDescMotor, cCodDestinado, 
                 ...
                 cColor, cSerialCarroceria, NULL, nNumExp,cClaseEmi, NULL, dFecIniVig, 
    ...
    FOR RA IN RAMOS LOOP 
      ...
      BEGIN 
        INSERT INTO CERT_RAMO(IdePol, NumCert, CodRamoCert, StsCertRamo, FecIniValid, 
                       ...
                       VALUES(nIdePol, nNumCert, cCodRamoCert, 'VAL', PR.FECHA_EMISION(SYSDATE), 
    ...
    BEGIN
      nNumPolDef:=PR_POLIZA.ACTIVAR(nIdePol,'D');
    END;
